...
//        }
        ...
        if($config['third_appid'] && $config['third_appid'] != $appid){
    ...
    /**
     * 消息解密
     * @param mixed $encryptMsg 微信传送过来的消息
    ...
    public function decryptMsg(){
        ...
        if ($encrypt_type == 'aes'){
            ...
            if($errCode != 0){
//                throw new \Exception('消息解密失败：'.$errCode);
            }
            $encryptMsg = $decryptMsg;
        }

        $xml = simplexml_load_string($encryptMsg, 'SimpleXMLElement', LIBXML_NOCDATA);
        return json_decode(json_encode($xml),true);
    }

    private function getPc(){
        if(is_null($this->wxBizMsgCrypt)){
            $this->wxBizMsgCrypt = new WXBizMsgCrypt($this->config['token'], $this->config['encoding_aes_key'], $this->config['appid']);
        }
    ...
    public function response($content, $type = self::MSG_TYPE_TEXT){
        ...
        if ($_GET['encrypt_type'] == 'aes'){
            $encryptMsg = ''; //加密后的密文
            $pc = $this->getPc();
            $pc->encryptMsg($xml, time(), $_GET['nonce'], $encryptMsg);
            $xml = $encryptMsg;
        }
        exit($xml);
    }
     ...
     * 转入多客服
     */
    public function replyCustomer($KfAccount = null){
        /* 转换数据为XML */
        $xml = '<xml>
        ...
        if(!empty($KfAccount)){
           $xml .= '<TransInfo><KfAccount><![CDATA['.$KfAccount.']]></KfAccount></TransInfo>';
        }
        $xml .= '</xml>';
        ...
        if ($_GET['encrypt_type'] == 'aes'){
            $encryptMsg = ''; //加密后的密文
            $pc = $this->getPc();
            $pc->encryptMsg($xml, NOW_TIME, $_GET['nonce'], $encryptMsg);
            $xml = $encryptMsg;
        }
        exit($xml);
    }
