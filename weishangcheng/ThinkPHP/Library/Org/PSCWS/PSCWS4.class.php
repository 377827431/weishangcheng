...
class PSCWS4
{
    ...
    function PSCWS4($charset = 'utf8')
    {
        ...
        if($charset == 'utf8' || $charset == 'utf-8'){
            $this->set_dict($path.'/etc/dict.utf8.xdb');
            $this->set_rule($path.'/etc/rules.utf8.ini');
        }else{
            $this->set_dict($path.'/etc/dict.xdb');
            $this->set_rule($path.'/etc/rules.ini');
        }
    }
    
    // FOR PHP5
    function __construct($charset = 'utf8') { $this->PSCWS4($charset); }
     ...
     * 获取分割后的字符串
     */
    function getText($text, $limit = 10, $xattr = 'n,v'){
        $this->send_text($text);
        $tops = $this->get_tops($limit, $xattr);
        
        $result = '';
        foreach ($tops as $item){
            $len = mb_strlen($item['word'], $this->_cs);
            if($len > 1){
                $result .= $item['word'].' ';
            }
        }
     ...
     * 获取分割后的字符串和拼音
     */
    public function getTextAndPinYin($text, $ignoreSingleWord = true){
        $this->send_text($text);
        $tops = $this->get_tops(10, 'n,v');
        
        $PY = NEW \Org\PinYin();
        
        $result = array('text' => array(), 'pinyin' => array());
        foreach ($tops as $item){
            $len = mb_strlen($item['word'], $this->_cs);
            if($len > 1){
                $result['text'][] = $item['word'];
            }
            
            $py = $PY->getAllPY($item['word']);
            if(strlen($py) > 1){
                $result['pinyin'][] = $py;
            }
        }
        
        $result['pinyin'] = array_diff($result['text'], $result['pinyin']);
        $result['text']   = implode(' ', $result['text']);
        $result['pinyin'] = implode(' ', $result['pinyin']);
        return $result;
    }
