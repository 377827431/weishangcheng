...
use Org\WxPay\WxPayReport;
use Org\WxPay\WxPayResults;
...
class WxPayApi
{
	...
	public static function orderQuery($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
		    ...
		    throw new WxPayException("订单查询接口中appid必填！");
		}else if(!$inputObj->IsMch_idSet()){
		    throw new WxPayException("订单查询接口中mch_id必填！");
		}
	...
	public static function refund($inputObj, $timeOut = 30)
	{
		...
		$xml = $inputObj->ToXml();
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, $inputObj->GetAppid(), $timeOut);
		$result = WxPayResults::Init($response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	...
	public static function notify($callback, &$msg)
	{
		...
		return call_user_func_array($callback, array($result, &$msg));
	}
	 ...
	 * @throws WxPayException
	 */
	private static function postXmlCurl($xml, $url, $certAppid = false, $second = 30)
	{		
		$ch = curl_init();
		//设置超时
		...
		*/
		...
		//设置证书
		if($certAppid !== false){
		    $certPath= __DIR__.'\\Apiclient\\'.$certAppid.'\\';
		...
		if($data){
			...
			$error = curl_errno($ch);
			curl_close($ch);
			if($error == 28 && $second == 30){ // 重试一次
				return self::postXmlCurl($xml, $url, $certAppid, 31);
			}
