...
//        $app_name       =   '';
        ...
        if(isset($_GET[$varPath])) { // 判断URL里面是否有兼容模式参数
        ...
        }else if(!isset($_SERVER['PATH_INFO'])) {// 分析PATHINFO信息
            ...
            foreach ($types as $type){
                if(0===strpos($type,':')) {// 支持函数判断
                    ...
                    substr($_SERVER[$type], strlen($_SERVER['SCRIPT_NAME']))   :  explode('?', $_SERVER[$type])[0];
                    break;
                }
            }
        }
        ...
        if(C('APP_SUB_DOMAIN_DEPLOY')) {
            ...
            if(!empty($rule)) {
                ...
                if(is_array($rule) && !isset($rule[0])){
                    ...
                    foreach ($rule as $pattern=>$module){
                        if(preg_match('/^'.$pattern.'$/', $array[0])){
                            if(is_array($module) || strpos($module, '/') !== false){
                                $rule = $module;
                            }else{
                                unset($rule);
                                define('BIND_MODULE', $module);
                                define('__APP__', '/'.$array[0]);
//                                if (in_array($array[0], self::$special)){
//                                    define('APP_NAME', self::$key);
//                                    $app_name = $array[0];
//                                }else{
                                    define('APP_NAME', $array[0]);
//                                }
                                $pathStart = 1;
                            }
                            break;
                ...
                if(is_string($rule) || isset($rule[0])){
        ...
        if(!defined('APP_NAME')){
//            if (in_array(MODULE_NAME, self::$special)){
//                define('APP_NAME', self::$key);
...
//            }
        ...
        if('' != $_SERVER['PATH_INFO'] && (!C('URL_ROUTER_ON') ||  !Route::check()) ){   // 检测路由规则 如果没有则按默认规则调度URL
            ...
            if($pathStart > 0){
                $paths = array_splice($paths, $pathStart);
            }
        ...
        // 获取控制器和操作名
//        if (in_array($app_name, self::$special)){
//            define('CONTROLLER_NAME',   $app_name);
//            define('ACTION_NAME',       defined('BIND_CONTROLLER')? BIND_CONTROLLER : self::getController($varController,$urlCase));
//        }else{
...
//        }
        ...
        define('__CONTROLLER__',__MODULE__.$depr.(defined('BIND_CONTROLLER')? $controllerName: ( $urlCase ? parse_name($controllerName) : $controllerName )) );
    ...
    static private function getController($var,$urlCase) {
        ...
        define('CONTROLLER_LOWER_NAME', strtolower($controller));
        if($urlCase) {
            // URL地址不区分大小写
