...
class Verify {
    ...
    public function entry($id = '') {
        ...
        if($this->useZh){ // 中文验证码
            ...
            $codeNX = mt_rand($this->fontSize/2, $this->fontSize*1.2); // 验证码第N个字符的左边距
            for ($i = 0; $i<$this->length; $i++) {
                $code[$i] = $this->codeSet[mt_rand(0, strlen($this->codeSet)-1)];
                imagettftext($this->_image, $this->fontSize, mt_rand(-30, 30), $codeNX, $this->fontSize*1.2, $this->_color, $this->fontttf, $code[$i]);
                $codeNX  += mt_rand($this->fontSize, $this->fontSize*1.5);
            }
        }
