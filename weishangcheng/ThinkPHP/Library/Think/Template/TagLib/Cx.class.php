...
class Cx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'breadcrumb'=>  array('attr'=>'class,split'),
        'toolbar'   =>  array('attr' => 'id,module'),
        );
    ...
    public function _breadcrumb($tag, $content){
        $breadcrumb = '<ul class="'.$tag['class'].'">'.$content;
        $breadcrumb .= '<?php 
                            $breadcrumb_list = \Common\Common\Permission::getParents();
            
                            $breadcrumb_count = count($breadcrumb_list) - 1;
                            $breadcrumb_i = 0;
                            foreach($breadcrumb_list as $item){
                                if($breadcrumb_i == $breadcrumb_count){
                                    echo \'<li class="active">\'.$item["title"].\'</li>\';
                                }else{
                                    if(!empty($item["module"]) && !empty($item["controller"])){
                                        $breadcrumb_url = U("/".$item["module"]."/".$item["controller"].($item["action"] == "index" ? "" : "/".$item["action"]), $item["params"]);
                                    }else{
                                        $breadcrumb_url = "javascript:void(0)";
                                    }
                                    echo \'<li><a href="\'.$breadcrumb_url.\'">\'.$item["title"].\'</a><span class="divider">'.$tag["split"].'</span></li>\';
                                }
                                $breadcrumb_i++;
                            }
                        ?>';
     ...
     * @return string
     */
    public function _toolbar($tag, $content){
        // 元素默认id
        if(empty($tag['id'])){
            $tag['id'] = 'toolbar';
        }
        
        $module = empty($tag['module']) ? MODULE_NAME.'.'.CONTROLLER_LOWER_NAME.'.'.ACTION_NAME : $tag['module'];
        $module = strtolower($module);
        $module = explode('.', $module);
        if(!isset($module[2])) $module[2] = 'index';
        ...
        $html .= '<?php \Common\Common\Auth::get()->showTollbar(\''.$module[0].'\', \''.$module[1].'\', \''.$module[2].'\') ?>';
        $html .= $content;
        $html .= '</div>';
        return $html;
    }
