...
class Redis {
	...
	public function __construct(){
        ...
        if(empty($options)) {
            $options = array (
                ...
                'timeout'       => true,
                'persistent'    => C('SESSION_PERSISTENT') ? : 'connect',
				'auth'			=> C('SESSION_REDIS_AUTH') ? : false,
            );
        }
	...
	public function connect($is_master = true) {
	    $isConnected = $this->handler->isConnected();
	    if($isConnected){
	        return;
	    }
		...
		$func = $this->options['persistent'] ? : 'connect';
		try {
	...
	public function read($sessID) {
		...
		if($this->get_result === false){
		    $this->write($sessID, $this->get_result);
		}
