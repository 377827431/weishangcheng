...
/**
 * session管理函数（已做如下调整：session使用户立即关闭）
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        // 启动session
        if(C('SESSION_AUTO_START') && !isset($_SESSION)){
            session_start();
            session_write_close();
        }
        return;
    ...
    if('' === $value){ 
        if(''===$name){
            // 获取全部的session
            $result = $prefix ? $_SESSION[$prefix] : $_SESSION;
            ...
            if(strpos($name,'.')){ // 支持数组
                ...
                $result = $prefix?isset($_SESSION[$prefix][$name1][$name2]):isset($_SESSION[$name1][$name2]);
            }else{
                $result = $prefix?isset($_SESSION[$prefix][$name]):isset($_SESSION[$name]);
            }
            ...
            if(strpos($name,'.')){
                list($name1,$name2) =   explode('.',$name);
                $result = isset($_SESSION[$prefix][$name1][$name2])?$_SESSION[$prefix][$name1][$name2]:null;  
            }else{
            ...
            if(strpos($name,'.')){
                list($name1,$name2) =   explode('.',$name);
                $result = isset($_SESSION[$name1][$name2])?$_SESSION[$name1][$name2]:null;  
            }else{
