 ...
 *
 */
class OrdernewController extends CommonController
{
    /**
     * 列表视图
     */
    ...
    public function search(){
        ...
        $projectIds = array();
        foreach ($list as $k => $v){
            if (!in_array($v['seller_id'], $shopIds)){
                $shopIds[] = $v['seller_id'];
            }
            $projecId = substr($v['seller_id'], 0, -3);
            if (!in_array($projecId, $projectIds)){
                $projectIds[] = $projecId;
            }
        }
        if (!empty($shopIds)){
        ...
        foreach ($list as $key => $value) {
            if($value['status'] == OrderStatus::WAIT_PAY){
                $list[$key]['buttons'] = array(
                    array('text' => '取消订单', 'class' => 'js-cancel-trade', 'url' => __MODULE__.'/ordernew/cancel'),
                    array('text'=>'微信付款','class'=>'wxzf','url'=>"javascript:;")
                );
                if(!empty($shop_info[$value['seller_id']]['pay_zfb']) && !is_null($shop_info[$value['seller_id']]['pay_zfb'])){
                    $list[$key]['buttons'] = array(
                        array('text' => '取消订单', 'class' => 'js-cancel-trade', 'url' => __MODULE__.'/ordernew/cancel'),
            ...
            if($value['status'] == OrderStatus::WAIT_CONFIRM_GOODS){
                $list[$key]['buttons'] = array(
                    array('text' => '查看物流', 'class' => 'js-search-express', 'url' => __MODULE__.'/ordernew/checkLogistics?tid='.$value['tid']),
                    array('text' => '确认收货', 'class' => 'js-confirm-goods', 'url'=> __MODULE__.'/ordernew/confirm')
                );
            }
        }
