...
class DiscountModel extends GoodsModel{
    protected $tableName = 'mall_discount';
    private $MainTag     = '限时折扣';
    private $PriceTitle  = '折扣价';
    private $ActivityTag = ActivityType::DISCOUNT;
    ...
    public function coverDetail($params, $goods, $buyer)
    {
        ...
        $sql = "SELECT activity.id AS activity_id, activity_name, activity.title, config.main_tag, config.price_title,
                    activity.price, activity.score, activity.total, activity.sold,
                    config.start_time, config.end_time, activity.min_order_quantity,
                    config.other_discount, config.level_quota, activity.buy_quota, activity.products
                FROM mall_discount AS activity
                INNER JOIN mall_discount_config AS config ON config.id=activity.config_id
        ...
        foreach ($goods['products'] as $i=>$product){
            ...
            $product['is_score'] = $goods['is_score'];
    ...
    public function coverGoodsList($activityGoodsList, $buyer, $settlement = false){
        ...
        $sql = "SELECT activity.id AS activity_id, goods_id, activity.title, price, score,
                    activity.total, activity.sold, config.main_tag, config.price_title,
                    config.start_time, config.end_time, activity_name, config.other_discount
                    ".($settlement ? ", activity.products, config.level_quota, activity.buy_quota, activity.min_order_quantity" : "")."
                FROM mall_discount AS activity
                INNER JOIN mall_discount_config AS config ON config.id=activity.config_id
    ...
    private function coverData($goods, $activity, $buyer){
        ...
        $goods['other_discount'] = $activity['other_discount'];
        ...
        if($goods['is_score']){
            $goods['activity']['description'] = '原价'.$goods['original_price'].'元';
        }else{
            $goods['activity']['description'] = '打'.(bcdiv($goods['price'], $goods['original_price'], 2) * 10).'折';
        }
    ...
    public function search($params, $login){
        ...
        if(is_numeric($params['shop_id'])){
            $where[] = "config.shop_id=".$params['shop_id'];
        }else if(is_numeric($params['project_id'])){
            $where[] = "config.shop_id BETWEEN {$params['project_id']}001 AND {$params['project_id']}999";
        }

        if($params['status'] == 'waiting'){
            $where[] = "config.end_time>".NOW_TIME." AND config.start_time>".NOW_TIME;
        }else{
            $where[] = NOW_TIME." BETWEEN config.start_time AND config.end_time";
        }
        ...
        $sql = "SELECT activity.id AS activity_id, goods.id AS goods_id, activity.title, goods.cat_id, goods.tag_id,
                    activity.price, activity.score, IF(goods.original_price > goods.price, goods.original_price, goods.price) AS original_price, goods.shop_id,
                    config.start_time, config.end_time, config.other_discount, config.level_quota, activity.buy_quota, activity.min_order_quantity,
                    goods.pic_url, activity.total, activity.sold, config.price_title, config.main_tag, config.hide_sold, config.hide_progress
                FROM mall_discount AS activity
                INNER JOIN mall_discount_config AS config ON config.id=activity.config_id
    ...
    public function syncStock($param){
        ...
        if(!empty($activity)){
            ...
            $this->execute("UPDATE mall_discount SET sold=sold-'{$param['quantity']}', products='{$products}' WHERE id={$id}");
        }
        $this->commit();
    }
}
