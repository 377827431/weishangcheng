...
class QrcodeModel extends Model{
    ...
    private function createRecommendImg($shop, $qrcodeFile, $folder, $type = 0){
        ...
        // 绘制背景图
        $image = imagecreatetruecolor(526*$resize, (723+$down)*$resize);
        ...
        if($shop['shopLogoFile']){
            ...
            imagecopyresized($image, $shopLogo, 58*$resize, 33*$resize, 0, 0, 80*$resize, 80*$resize, $psizearray[0], $psizearray[1]);
        ...
        if($shop['shopSignFile']){
            ...
            imagecopyresized($image, $shopSign, 58*$resize, 146*$resize, 0, 0, 410*$resize, 409*$resize, $psizearray[0], $psizearray[1]);
        ...
        if($shop['longTouchFile']){
            ...
            imagecopyresized($image, $longTouchFile, 389*$resize, (593+$down)*$resize, 0, 0, 89*$resize, 74*$resize, $psizearray[0], $psizearray[1]);
        ...
        imagecopyresized($image, $img_qrcode, 58*$resize,  (589+$down)*$resize, 0, 0, $psizearray[0], $psizearray[1], $psizearray[0], $psizearray[1]);
        ...
        if ($length > 8){
            ...
            imagefttext($image,21*$resize,0, (254-92)*$resize, (85-15)*$resize, $blackcolor, $font, $name);
            $name2 = mb_substr($str, 8, $length - 8, 'utf-8');
            imagefttext($image,21*$resize,0, (254-92)*$resize, (85+15)*$resize, $blackcolor, $font, $name2);
        }else{
            $name = $str;
            imagefttext($image,21*$resize,0, (254-92)*$resize, 85*$resize, $blackcolor, $font, $name);
        }
        //长按或扫描二维码
        imagefttext($image,15*$resize,0, 158*$resize, (625+$down)*$resize, $gray, $font, '长按或扫描二维码');
        //查看详情
        if ($type == 1){
            ...
            imagefttext($image,17*$resize,0, 58*$resize, 605*$resize, $gray, $font, $title);
            if ($dt != 0){
                imagefttext($image,17*$resize,0, 58*$resize, (605+28)*$resize, $gray, $font, $title2);
            }
            imagefttext($image,15*$resize,0, 160*$resize, (653+$down)*$resize, $gray, $font, '查看商品');
        }else{
            imagefttext($image,15*$resize,0, 160*$resize, 653*$resize, $gray, $font, '查看店铺');
        }
        // 保存图像
    ...
    private function createRecommendImg2($shop, $qrcodeFile, $folder, $type = 0){
        ...
        $white2 = imagecolorallocatealpha($image, 255, 255, 255, 30);
...
//        }
        ...
        for($x = (180*$resize - $shopLogoSize)/2; $x < (218*$resize + $shopLogoSize)/2; $x++){        
            for($y = $shopLogoTop; $y < $shopLogoTop + $shopLogoSize; $y++){        
                if ((($x - $cx) * ($x - $cx) + ($y - $cy) * ($y - $cy)) > (($shopLogoSize-2) * ($shopLogoSize-2))/4){
                    imagesetpixel($image,$x,$y,$white2);
                }       
            }       
        }
        ...
        if ($length > 8){
            $name = mb_substr($str, 0, 8, 'utf-8');
            imagefttext($image,21*$resize,0, (268-92)*$resize, (70-15)*$resize, $gray, $font, $name);
            $name2 = mb_substr($str, 8, $length - 8, 'utf-8');
            imagefttext($image,21*$resize,0, (268-92)*$resize, (70+15)*$resize, $gray, $font, $name2);
        }else{
            $name = $str;
            imagefttext($image,21*$resize,0, (268-92)*$resize, 70*$resize, $gray, $font, $name);
        }
        //商品价格
        if($shop['price']){
        ...
        //长按或扫描二维码
        imagefttext($image,15*$resize,0, 222*$resize, (560+$down)*$resize, $gray, $font, '长按或扫描二维码');
        if ($type == 1){
            $title = mb_substr($title_str, 0, $dt == 0 ? $title_length : $dt, 'utf-8');
            if ($dt != 0){
                $title2 = mb_substr($title_str, $dt, $title_length - $dt, 'utf-8');
            ...
            //商品名称
            imagefttext($image,17*$resize,0, 74*$resize, 500*$resize, $gray, $font, $title);
            if ($dt != 0){
                imagefttext($image,17*$resize,0, 74*$resize, (500+28)*$resize, $gray, $font, $title2);
            }
            //查看详情
            imagefttext($image,15*$resize,0, 222*$resize, (600+$down)*$resize, $gray, $font, '查看详情');
        }else{
            imagefttext($image,15*$resize,0, 222*$resize, 600*$resize, $gray, $font, '查看店铺');
        }

//        imageantialias($image, true);
        $shop['shopLogoFile'] = 'C:\Users\jy\Desktop\WORKSPACE\PHP\WeiShangCheng\Public/img/mall/ddpp.jpg';
        // 读取头像图片
        if($shop['shopLogoFile']){
            $shopLogo = imagecreatefrompng($shop['shopLogoFile']);
            if (!$shopLogo){
            ...
            $psizearray = getimagesize($shop['shopLogoFile']);
            imagecopyresized($image, $shopLogo, 240*$resize, 425*$resize, 0, 0, 130*$resize, 130*$resize, $psizearray[0], $psizearray[1]);
            imagedestroy($shopLogo);
        }



        $ceshi = 'C:\Users\jy\Desktop\WORKSPACE\PHP\WeiShangCheng\Public/img/mall/dp.png';

        if($ceshi){
            $shopLogo = imagecreatefrompng($ceshi);
//            imageantialias($shopLogo, true);
            if (!$shopLogo){
                $shopLogo = imagecreatefromjpeg($ceshi);
            }
            $psizearray = getimagesize($ceshi);
            imagecopyresized($image, $shopLogo, 0*$resize, 0*$resize, 0, 0, 620*$resize, 1105*$resize, $psizearray[0], $psizearray[1]);
            imagedestroy($shopLogo);
        }
