...
class RefundModel extends \Common\Model\BaseModel
{
    ...
    public function getAll($paging){
        ...
        if($_GET['start_date'] != '' && $_GET['end_date'] != ''){
            $where[] = " refund.refund_created between '".strtotime($_GET['start_date'])."' AND '".strtotime($_GET['end_date'])."'";
        }
        ...
        if($paging==0){
            ...
            $sql = "SELECT `order`.tid, `order`.goods_id, `order`.sku_json, `order`.title, `order`.quantity AS buy_num, `order`.price, `order`.pic_url,
            refund.refund_created, refund.refund_reason, refund.refund_fee, refund.refund_quantity, refund.refund_post, refund.refund_express, refund.refund_status,trade.seller_nick,buyer_id
            FROM {$this->tableName} AS refund
            INNER JOIN trade_order AS `order` ON `order`.oid=refund.refund_id
            INNER JOIN trade AS trade ON trade.tid=`order`.tid
            {$where}
