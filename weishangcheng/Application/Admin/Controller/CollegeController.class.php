...
class CollegeController extends CommonController
{
    ...
    private function showList(){
        ...
        $sql = "SELECT * FROM mall_college {$where} LIMIT {$offset}, {$limit}";
        $Model=M();
        ...
        $data['rows'] = $list;
        $data['total'] = count($Model->query("SELECT * FROM mall_college"));
    ...
    public function delete(){
        ...
        $id = addslashes($id);
        M()->query("DELETE from mall_college where id in ($id)");
        $this->success('已删除');
    }
}
?>